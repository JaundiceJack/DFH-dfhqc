// Import action types
import {
  GET_LOTS, SELECT_LOT, LOT_ADDED, LOT_EDITED, LOT_DELETED,
...
export const addLot = (lot) => (dispatch, getState) => {
  // Set Headers
  const config = { headers: {"Content-type": "application/json"} };
  ...
  .then(res => {
    dispatch({type: LOT_ADDED, payload: res.data});
    dispatch(getTests(getState().lot.selectedLot._id));
  })
  .catch(err => { handleError(err) });
}
export const editLot = (lot) => (dispatch, getState) => {
  // Set Headers
  const config = { headers: {"Content-type": "application/json"} };
  // Convert the new lot to JSON for sending
  ...
  .then(res => {
    dispatch({type: LOT_EDITED, payload: res.data});
    dispatch(getTests(getState().lot.selectedLot._id));
  })
  .catch(err => { handleError(err) });
}
export const deleteLot = (lot) => dispatch => {
  ...
  .then(res => {
    dispatch({ type: LOT_DELETED, payload: lot })
  })
  .catch(err => { handleError(err) });
}
