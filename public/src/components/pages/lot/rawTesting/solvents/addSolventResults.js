...
const AddSolventResults = ({ lotId, sample, spec, takeResults, close }) => {
  ...
  return (
    <form className="flex flex-col px-2 pb-4" onSubmit={e => takeResults(e, current)}>
      <p className="text-center mb-2">Testing solvents...</p>
      <Selection label="Testing Lab:" name="lab" value={current.lab}
        ...
        append={
          <Entry type="date" name="date_sent" value={current.date_sent}
            ...
            extraClasses="ml-2 " /> } />
      <Selection label="Solvents:" name="presence" value={current.results.presence}
...
export default AddSolventResults;
