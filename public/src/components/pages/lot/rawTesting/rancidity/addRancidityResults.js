...
const AddRancidityResults = ({ lotId, sample, spec, takeResults, close }) => {
  ...
  const [current, setCurrent] = useState({
    ...
    results: {
      peroxide:  (sample && sample.results && sample.results.peroxide !== "" && sample.results.peroxide !== null) ?
        sample.results.peroxide  : "",
      anisidine: (sample && sample.results && sample.results.anisidine !== "" && sample.results.anisidine !== null) ?
        sample.results.anisidine : "",
    },
  ...
  return (
    <form className="flex flex-col px-2 pb-4" onSubmit={e => takeResults(e, current)}>
      <p className="text-center mb-2">Testing rancidity...</p>
      <Selection label="Testing Lab:" name="lab" value={current.lab}
        ...
        append={
          <Entry type="date" name="date_sent" value={current.date_sent}
            ...
            extraClasses="ml-2 " /> } />
      <Entry type="number" label="Peroxide:" name="peroxide" value={current.results.peroxide}
        onChange={e => setCurrent({...current,
          results: { ...current.results, peroxide: e.target.value },
          date_of_result: new Date().toISOString().split('T')[0]
        })}
        append="meq/kg" />
      <Entry type="number" label="p-Anisidine:" name="anisidine" value={current.results.anisidine}
        onChange={e => setCurrent({...current,
          results: { ...current.results, anisidine: e.target.value },
          date_of_result: new Date().toISOString().split('T')[0]
        })}
        append="meq/kg" />
...
export default AddRancidityResults;
