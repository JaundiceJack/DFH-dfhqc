...
const HmTests = ({ lot }) => {
  ...
  const takeResults = (e, sample) => {
    ...
    const converted = { ...sample, results: { ...sample.results,
      arsenic: sample.results.arsenic !== "" ? Number(sample.results.arsenic) : null,
      cadmium: sample.results.cadmium !== "" ? Number(sample.results.cadmium) : null,
      lead:    sample.results.lead    !== "" ? Number(sample.results.lead)    : null,
      mercury: sample.results.mercury !== "" ? Number(sample.results.mercury) : null,
      nickel:  sample.results.nickel  !== "" ? Number(sample.results.nickel)  : null,
    } };
    dispatch(editRawSample(lot._id, 'hm', converted));
    ...
    <Container title="Heavy Metals" loading={loading} contents={
      ...
      {(sampling !== 'hm') && (testing !== 'hm') && (deleting !== 'hm') &&
        <div className="pb-2">
          ...
          <SampleInfo sample={currentSample} spec={lot.raw.hm} />
        </div>
      }
