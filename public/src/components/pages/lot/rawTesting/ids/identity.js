...
import AddIdentityResults from './addIdentityResults.js';
...
const Identity = ({ lot, spec }) => {
  ...
  const currentTestType = tests ?
    tests.filter(test => (test.type === 'identity' && test.identity === spec.identity._id)) :
    null;
  ...
  const takeSample = (e, sample) => {
    e.preventDefault();
    dispatch(takeRawSample(lot._id, 'identity', sample, null, spec.identity._id));
  ...
  const takeResults = (e, sample) => {
    e.preventDefault();
    dispatch(editRawSample(lot._id, 'identity', sample, null, spec.identity._id));
  ...
  return (
    <div style={{minHeight: 100+'px'}}
      ...
      <div>
        <div className="pb-2">
          {(testing !== spec.identity._id) &&
           (sampling !== spec.identity._id) &&
           (deleting !== spec.identity._id) &&
          <div>
            <OptionRow type={spec.identity._id} test={currentTest}
              showSampling={sampling === spec.identity._id}
              showTesting={testing === spec.identity._id}
              showDeleting={deleting === spec.identity._id} />
            <SelectSample type={spec.identity._id} test={currentTest}
        ...
        {(sampling === spec.identity._id) &&
          <AddSample lotId={lot._id}  takeSample={takeSample} testName={spec.identity.name}
            close={() => dispatch(setShowSampling(''))} />
        }
        {(testing === spec.identity._id) &&
          <AddIdentityResults lotId={lot._id}
