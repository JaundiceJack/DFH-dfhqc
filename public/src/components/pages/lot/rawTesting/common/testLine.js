...
const TestLine = ({
  ...
  const textColor = () => {
    ...
    else if (spec && sample && sample.lab && (result === null || result === "")) return "text-indigo-200"

    // Green if results passing, red if results failing
    else if (spec && sample && sample.lab && result !== null && result !== "") {
      ...
      // TODO: If an identity, check that it matches the posneg property of the spec
        ...
        return result === 'positive' ? "text-green-200" : "text-red-400";
      }
    }
    else
      return "text-white"
  ...
  const testText = () => {
    ...
    else if (spec && sample && sample.date_sampled && sample.lab && (result === null || result === ""))
      return `Sent to ${capitalize(sample.lab.name)} on ${formatDate(sample.date_sent)}`
    // Text is the result if off test
    else if (spec && sample && sample.date_sampled && sample.lab && result !== "" && result !== null)
