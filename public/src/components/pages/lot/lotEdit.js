...
const LotAdd = ({toggleEdit}) => {
  ...
  const [lotVals, setLotVals] = useState({
    ...
    rawId:     selected.item_type === 'raw ' ? selected.item._id :
      (!raws.loading && raws.length > 0) ? raws[0]._id : "",
    blendId:   selected.item_type === 'blend ' ? selected.item._id :
      (!blends.loading && blends.length > 0) ? blends[0]._id : "",
    bulkId:    selected.item_type === 'bulk ' ? selected.item._id :
      (!bulks.loading && bulks.length > 0) ? bulks[0]._id : "",
    fgId:      selected.item_type === 'fg ' ? selected.item._id :
      (!fgs.loading && fgs.length > 0) ? fgs[0]._id : "",
