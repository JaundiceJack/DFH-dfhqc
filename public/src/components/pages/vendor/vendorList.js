...
import { selectVendor } from '../../../actions/vendorActions';

const VendorList = () => {
  // Get the items from the server upon loading
  const items = useSelector(state => state.vendor.vendors);
  const selectedId = useSelector(state => state.vendor.selectedVendor._id);
  ...
  const rowClick = (vendor) => { dispatch(selectVendor(vendor)) }
